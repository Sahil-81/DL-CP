...
def main():
    ...
    if selected_box == 'About the App':
        readme=Image.open('/content/drive/MyDrive/readme_app.PNG')
        st.image(readme)
...
def models():
    ...
    if selection=='Predict on sample Images':
        ...
        if option=='<select>':
            ...
            path = os.path.join(os.getcwd(),'/content/drive/MyDrive/NoisyImage/')
...
#st.cache
def get_model():
    RIDNet=tf.keras.models.load_model('/content/drive/MyDrive/RIDNet.h5')
    return RIDNet
